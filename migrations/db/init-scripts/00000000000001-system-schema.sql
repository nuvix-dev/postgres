...
BEGIN
    ...
    -- insert into metadata table, raise error if type is different
    BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
        ...
        IF EXISTS (
            SELECT 1 FROM system.schemas WHERE name = schema_name AND type <> schema_type
        ) THEN
            RAISE EXCEPTION 'Schema "%" already exists with a different type.', schema_name;
        ELSE
            -- If type is same, update description and updated_at
            UPDATE system.schemas SET description = schema_description, updated_at = NOW()
            WHERE name = schema_name;
            SELECT id INTO new_schema_id FROM system.schemas WHERE name = schema_name;
        END IF;
    END;
